...
func TestM2bmFTDefaultDialectAndAggregateCap(t *testing.T) {
	...
	asserts.AssertStatusReply(t, db.Exec(nil, utils.ToCmdLine("FT.CONFIG", "SET", "MAXAGGREGATERESULTS", "2")), "OK")
	...
	ag, ok2 := agg.(*FTAggregateReply)
	if !ok2 {
		t.Fatalf("AGGREGATE type %T", agg)
	}
	mb, ok2 := ag.resp2.(*protocol.MultiBulkReply)
	if !ok2 {
		t.Fatalf("AGGREGATE type %T", agg)
	}
	...
	_ = db.Exec(nil, utils.ToCmdLine("FT.CONFIG", "SET", "MAXAGGREGATERESULTS", "10000"))
