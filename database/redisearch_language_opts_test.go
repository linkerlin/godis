...
func TestFTLanguageReturnInkeysLoadParams(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{"dialect-miss", []string{"FT.SEARCH", "p80", "hello", "DIALECT"}, "SEARCH_PARSE_ARGS Need an argument for DIALECT"},
		{"dialect-0", []string{"FT.SEARCH", "p80", "hello", "DIALECT", "0"}, "SEARCH_PARSE_ARGS DIALECT requires a non negative integer >=1 and <= 4"},
		{"dialect-5", []string{"FT.SEARCH", "p80", "hello", "DIALECT", "5"}, "SEARCH_PARSE_ARGS DIALECT requires a non negative integer >=1 and <= 4"},
		...
		{"filter-miss", []string{"FT.SEARCH", "p80", "hello", "FILTER"}, "SEARCH_PARSE_ARGS FILTER requires 3 arguments"},
		{"geofilter-miss", []string{"FT.SEARCH", "p80", "hello", "GEOFILTER"}, "SEARCH_PARSE_ARGS GEOFILTER requires 5 arguments"},
