...
| FT.SEARCH DIALECT/SORTBY/FILTER/GEOFILTER/SLOP/PAYLOAD/INFIELDS/EXPANDER | ✅ 子集 | 缺参/范围文案对齐 Redis 8.x；SORTBY 未知→`Property … not loaded nor in schema`；GEOFILTER `<lon/lat/radius>`/`Unknown distance unit`/`Invalid GeoFilter lat/lon`；FILTER `Bad lower/upper range`；SLOP 负值接受；EXPANDER 任意名；**BM25STD_TANH_FACTOR** 缺参/`1..10000` 文案；AGGREGATE APPLY 缺参/`AS needs argument`/无 AS；GROUPBY nargs |
