...
| FT.SEARCH 同义词展开 | ✅ 查询词按 FT.SYNUPDATE 分组展开为 `term \| syn1 \| syn2`（不含短语内部展开）；**FT.SYNADD** 对齐 Redis 8.10：`No longer supported, use FT.SYNUPDATE` |
...
| FUNCTION DUMP 官方互通 | Godis `GODISFN1` 自洽（含多库 FLUSH/REPLACE/APPEND 往返测）；**显式拒绝**官方 `0xF5`/`0xF6`/`REDIS####`；截断/异己二进制明确 ERR；兼旧文本 | 见「官方 FUNCTION DUMP 边界」；**禁止假互通** |
| 完整 BM25 / 完整 KNN 方言 / 完整 DIALECT | BM25STD：WEIGHT + 文档长度 + **可测 IDF** + **多字段加权求和**；NORM min-max；**TANH + BM25STD_TANH_FACTOR**（缺参/`must be between 1 and 10000`；整数 >10000 仍接受）；**FT.CONFIG**（`SEARCH_PARSE_ARGS`/`SEARCH_VALUE_BAD`/`SEARCH_OPTION_INVALID`/`EXCESSARGS`；**ON_TIMEOUT RETURN-STRICT**；HELP 空体）；**FT._LIST** 字典序；**FT.ALIASADD** `SEARCH_INDEX_EXISTS`/`SEARCH_ALIAS_CONFLICT`；KNN：`AS`/`$YIELD_DISTANCE_AS`、**HYBRID_POLICY**（子句与 **`$HYBRID_POLICY` 属性块**）、**BATCH_SIZE**（接受/缺值 ERR）、**EPSILON**（距离阈值）、**`$EF_RUNTIME`** 属性块、空预过滤；**VECTOR_RANGE**（DIALECT≥2；**`$radius` PARAM**；**`$YIELD_DISTANCE_AS`**；**`$EPSILON` 仅 HNSW/SVS**（FLAT→Invalid option）；拒 **`$EF_RUNTIME`/hybrid 属性/未知键**；`($r)` Redis 拒→不实现）；**查询属性 `$weight`**（乘分子集）+ `$slop`/`$inorder`；拒非法/未知/`$phonetic:true`；**FORMAT**（STRING；拒 JSON；EXPAND→DIALECT≥3+RESP3+ON JSON）；**FT.AGGREGATE 多字段 SORTBY** + **SORTBY … MAX**；**LOAD … AS** / **`@__key`** ；**无 LOAD 仅 SORTABLE**（对齐 Redis 空行）；**FT.CREATE FILTER**；**WITHCURSOR MAXIDLE** + **COUNT** `Bad arguments for COUNT:…`；**SCORER** 校验（缺参/`No such scorer`；大小写敏感）；**FT.TAGVALS**（`No such field`/`Not a tag field`）；**LANGUAGE**（缺参/`No such language`；大小写不敏感）；**RETURN/INKEYS/LOAD/PARAMS** `SEARCH_PARSE_ARGS` 文案 + 负值边界；**HIGHLIGHT** 缺参；**GEOFILTER** `SEARCH_PARSE_ARGS` `<lon/lat/radius>`/`Unknown distance unit`/`requires 5` + **`SEARCH_SYNTAX Invalid GeoFilter lat/lon`**；**FILTER** `Bad lower/upper range`/`requires 3`；**SLOP** convert（负值接受）；**EXPANDER** 缺参/任意名；**AGGREGATE APPLY**（缺参/`AS needs argument`；无 AS→别名=表达式）+ **GROUPBY** nargs 文案 + AGGREGATE **FILTER** 缺参；**SEARCH_ARG_UNRECOGNIZED** 未知选项位次；裸 **WITHCOUNT**；裸 **REDUCE→UNRECOGNIZED**；CREATE **STOPWORDS/TEMPORARY** PARSE_ARGS（**TEMPORARY 0** 立即无索引）；CREATE **PREFIX/ON/LANGUAGE/SCORE/INDEXALL/FILTER**/无 SCHEMA 文案；**SCORE**∈[0,1]|NaN；**LANGUAGE_FIELD/SCORE_FIELD/PAYLOAD_FIELD** 缺参；SCHEMA **Fields arguments are missing** / **Field type** / **WEIGHT**/SEPARATOR/PHONETIC/VECTOR algo·count 文案；**FT.ALTER** `ALTER must be followed by SCHEMA` / `Unknown action` / `SEARCH_QUERY_BAD Duplicate field` / **SKIPINITIALSCAN**；**DROPINDEX** 未知参 `SEARCH_ARG_UNRECOGNIZED Unknown argument`；**FT.SYNADD** `No longer supported, use FT.SYNUPDATE`；SYNUPDATE/SYNDUMP 缺索引 `INDEX_NOT_FOUND`；**TIMEOUT** `SEARCH_PARSE_ARGS` 前缀；**CURSOR READ COUNT** `Bad value for COUNT`/负值排空；**TIMEOUT** 非法/缺参文案 + **soft 中途取消保留部分结果** / **ON_TIMEOUT RETURN|FAIL**（CONFIG）；DIALECT 1/2/3 子集（GEOSHAPE≥3；**tag 空格/多字段 `@f1\|f2` 强制 ≥2**）；AGGREGATE APPLY：UTC 时间族、`parsetime`/`geodistance`、**`matched_terms([max])` 多值**、**`split` 字符集 sep/strip**、**`format` PARSE_ARGS**、**`to_str`/`to_number`**；**FT.AGGREGATE ADDSCORES**（`@__score`；BM25STD 自洽）；**FT.SEARCH WITHOUTCOUNT**；**EXPLAINSCORE**（需 WITHSCORES；BM25/TFIDF/DOCSCORE/DISMAX 子集；b 与 Redis 默认不同）+ **SORTBY WITHCOUNT** 接受 | **非**论文级完整 BM25（无 slop 罚分进打分、无全局 NORM over collection、无 stemmer 变体进 `matched_terms`）；`$weight` 非 Redis 字节级项贡献；EXPAND 非完整 JSON 树；非完整 KNN/DIALECT 4；VECTOR_RANGE 无 HNSW 近似扫描语义；DIALECT 4 排序优化仍未完整；ADDSCORES/EXPLAINSCORE 非字节级对齐 RediSearch；见 REDISEARCH_ALIGNMENT |
...
| CI Redis sidecar 全量输出 diff（R4-1） | 未做全量 | **扩大 allowlist**（`scripts/r4-1-cases.txt`）：String/Hash/List/Set/ZSet/TTL + Stream/Geo/Bitops/HLL lite + **BITOP DIFF/DIFF1/ANDOR/ONE** + COPY/RENAMENX/LCS/MSETNX/LPUSHX + **SETNX/RPUSHX/ZREVRANK/BITPOS** + **INCRBYFLOAT/TOUCH/RENAME/EXPIREAT/GETEX/EXPIRETIME/LPOS/LMOVE/ZREMRANGEBYSCORE/PSETEX** + **ZRANGE/HMGET/LRANGE/GETDEL/ZRANK** + **ZMSCORE/ZREVRANGE/SMISMEMBER/HINCRBYFLOAT/PEXPIRETIME/ZDIFFSTORE** + **ZPOPMIN/ZPOPMAX（WANT `\n`）/ZLEXCOUNT/PFMERGE/ZRANGEBYSCORE** + **ZRANGESTORE/ZRANGEBYLEX/BITFIELD/HEXPIRE+HTTL** + **ZREMRANGEBYLEX/HPEXPIRE+HPTTL** + **SDIFF/ZDIFF/XTRIM/ZMPOP/LMPOP** + **SINTER/STRLEN/APPEND/GETRANGE/DECRBY** + **SUNION/ZREMRANGEBYRANK/LREM/HSETNX** + **ZREVRANGEBYSCORE/RPOPLPUSH/MGET** + **HSTRLEN/UNLINK/RENAMENX/SINTERCARD/ZINTERCARD/LINSERT/LSET/RPOP** + **ZINTER/ZUNION 单成员、MOVE、LINDEX** + **GEOHASH、SPOP 单成员、HKEYS/HVALS 单字段** + **单成员 ZRANDMEMBER/HRANDFIELD/SRANDMEMBER** + **GEORADIUS/GEOSEARCH ASC COUNT 1** + **LCS LEN、ZRANGE/ZREVRANGE WITHSCORES、EXPIRE NX/XX/GT/LT、SISMEMBER、GETRANGE 负/正索引、ZADD NX/XX/CH/GT/LT/INCR、BITCOUNT 字节范围、SREM、OBJECT ENCODING、SET GET、EXISTS 多键、LTRIM、ZCOUNT/ZINCRBY、HMSET、PEXPIREAT** + 第七十九批 APPEND/STRLEN/LREM/ZRANK/ZREM/HINCRBYFLOAT/SISMEMBER/BITCOUNT/XTRIM + **第八十批 GETSET/TYPE none/EXPIRE+TTL/HDEL·SREM 多成员/LRANGE 负索引/ZCOUNT/TOUCH/RENAMENX 目标已存在→0/PEXPIRE+PTTL/MSET** + **第八十一批 TYPE 分型/HEXISTS·HLEN/ZRANK·ZCARD/SCARD/LLEN/DECRBY/GETBIT/PFCOUNT/COPY/SETEX·PERSIST/EXISTS 多键/UNLINK/SETNX** + **第八十二批 DECR/INCRBY/HINCRBY/ZINCRBY/SUBSTR/LPOP·RPOP/SREM·ZREM/APPEND·GETDEL/MSETNX·RENAMENX/TOUCH/BITOP NOT/XADD·XDEL/PFADD/HSETNX/SETRANGE/PSETEX/ECHO/DBSIZE/OBJECT REFCOUNT** + **第八十三批 BITOP OR/XOR、LINDEX/LSET/LINSERT、GETEX、STRLEN/XLEN miss、APPEND、HSTRLEN、SINTERCARD/ZINTERCARD、ZREMRANGEBYRANK、ZLEXCOUNT、RPUSHX、BITPOS、PFCOUNT multi、INCRBYFLOAT** + **第八十四批 LPUSHX、GETBIT、miss HLEN/HEXISTS/SCARD/LLEN/EXISTS/UNLINK、SDIFFSTORE/SUNIONSTORE/SINTERSTORE、ZUNIONSTORE/ZSCORE、GETRANGE、SETEX/PERSIST、DECR/INCRBY/HINCRBY、BITOP AND** + **第八十五批 miss GETBIT/COPY/PFCOUNT/BITCOUNT/BITPOS/PTTL/PERSIST/EXPIRETIME/MOVE、HSETNX exist、SMOVE/ZCOUNT/HSTRLEN/LREM/ZREM/HDEL miss、SETRANGE/INCR/DECR/APPEND 新建、SETNX exist、SINTERCARD/ZINTERCARD 0、UNLINK/TOUCH 多 miss、BITFIELD GET、ZINCRBY/HINCRBY 新建、SUBSTR/LCS LEN/COPY hit/GETEX/XADD·XLEN·XDEL/PFADD·PFCOUNT/SUNIONSTORE** + **第八十六批 miss SCARD/LLEN/ZCARD、HGET miss、MGET 部分 miss、BITOP OR、PFADD 幂等 PFCOUNT** + **第八十七批 miss ZSCORE/ZRANK/ZREVRANK/ZMSCORE/ZPOPMIN、GETDEL/LMOVE/LPOS/GETRANGE/STRLEN、OBJECT IDLETIME、SET GET、BITFIELD SET、HINCRBYFLOAT/INCRBYFLOAT 新建、GETSET create、SETRANGE create、LMOVE/LPOS hit、ZREMRANGEBYSCORE、SMISMEMBER、XTRIM、DECRBY、TTL/TYPE/EXISTS miss、DBSIZE** 等稳定有序路径；Redis allowlist **1254** 条（含第七十八–九十一批扩表；单测 + Redis 8.10 WANT 探测；非全量）；sidecar WANT **`$-1`/`$empty`** 对齐 `--raw` 空值；`check_want` **`<=` 边界**（保存 bound，免 `is_int` 冲掉 `BASH_REMATCH`）；`@skip`/`@todo` 诚实标注已知洞；**仍非** FT/模块/DUMP/集群/无序回复全量套件 |
...
| 已知 skip/todo | `GEODIST`/`GEOPOS` 浮点与嵌套 `--raw`（不做脆弱精确浮点）；`XRANGE` 嵌套布局；大基数 `PFCOUNT` 近似漂移；XGROUP/XREAD BLOCK |
...
| KNN | `*=>[KNN …]` / 预过滤；`AS` 与 **`=>{$YIELD_DISTANCE_AS}`**；`EF_RUNTIME` / **`$EF_RUNTIME`**；`HYBRID_POLICY` / **`$HYBRID_POLICY`**∈{ADHOC_BF,BATCHES}（均暴力）；**`BATCH_SIZE`**（缺值 ERR；正整数存储、standalone 忽略）；**`EPSILON`**（缺值 ERR；可解析正数→距离 `<ε` 过滤）；**`$SHARD_K_RATIO`∈(0,1]** 校验后 standalone 忽略；空候选→0 | 真 BATCHES 迭代；`$SHARD_K_RATIO` 集群语义；HNSW 近似保证≠暴力；EPSILON 非 RediSearch 全方言等价 |
| VECTOR_RANGE | `@vec:[VECTOR_RANGE r|$r $q]`（DIALECT≥2）；**`$YIELD_DISTANCE_AS`**；**`$EPSILON` 仅 HNSW/SVS**（FLAT→`Invalid option`；ε 扩有效半径 `r*(1+ε)`）；拒 **`$EF_RUNTIME`**、**`$HYBRID_POLICY`/`$BATCH_SIZE`**（非 hybrid）、未知属性；FLAT 暴力距离过滤 | HNSW 候选扩展扫描；`($r)` 括号形 Redis 拒；与官方分数/半径边界字节级对齐 |

| DIALECT | 1/2 优先级；2：PARAMS/KNN/**VECTOR_RANGE**/比较/`ismissing`/tag 空格/`@f1\|f2`；3：GEOSHAPE；**WITHOUTCOUNT** 领先总数=本页文档数；**EXPLAINSCORE**（需 WITHSCORES）；**SORTBY WITHCOUNT** 接受；非法值与具体 ERR | DIALECT 3 多值 JSON 全量返回；DIALECT 4 排序优化；EXPLAINSCORE 非字节级对齐 |
| AGGREGATE APPLY / ADDSCORES | 算术/字符串/布尔函数；UTC `day/hour/minute/month/dayof*`、`year/monthofyear`；strftime 子集 `timefmt`/`parsetime`（**`%Y/%m/%d/%H/%M/%S/%y/%F/%T/%R/%a/%A/%b/%B/%I/%p/%P/%k/%l/%z/%w/%j/%e + %s/%u/%D/%x/%X/%r/%c/%C/%G/%g/%V/%U/%W/%h/%Z/%n/%t`**；**未知→Null**；%Z→UTC；`parsetime` 失败→Null/`$-1`；对照 Redis **8.6**）；两/三/四参 `geodistance`（含 point+lon+lat）；**`matched_terms([max])` 多值**（无 stemmer 变体）；**`split` 真多值**（**charset sep/strip**、空段丢弃）；**`format` PARSE_ARGS**；**`to_str`/`to_number`**；**缺字段 `SEARCH_VALUE_NOT_FOUND`**（**`exists()`**；**`&&`/`||`/`case` 短路**；**FILTER/APPLY 命令序交错**（多 FILTER；APPLY 间 FILTER）+ **数值串/布尔 coerce** + **字符串类型校验**（upper/lower 非串→Null；strlen/startswith/contains/split/substr 非串→Invalid type；负偏移越界→空）+ **FILTER≺APPLY 先裁剪**）；**`substr` 字节偏移**；**ADDSCORES→`@__score`**（BM25STD）；**多字段 SORTBY** + **SORTBY … MAX**；**LOAD … AS** / **`@__key`**（有 LOAD 时投影；无 LOAD 仅 SORTABLE）；**APPLY/FILTER 未入管道→`Property not loaded nor in pipeline`**（GROUPBY/REDUCE 仍可读全文档）；**REDUCE 名称/参数校验**（`No such reducer`/`Count accepts 0…`/`Missing arguments`/`QUANTILE`/`RANDOM_SAMPLE`/`FIRST_VALUE BY`）；**FT.CREATE FILTER**；**WITHCURSOR MAXIDLE/COUNT**；**SCORER** 校验；**FT.TAGVALS** 字段类型；**LANGUAGE**/RETURN/INKEYS/LOAD/PARAMS/HIGHLIGHT/CURSOR READ COUNT；**TIMEOUT soft 部分结果** + **ON_TIMEOUT RETURN\|FAIL** | 完整 strftime/本地化；stemmer 形态进 matched_terms；DIALECT 4 排序优化仍未完整；ADDSCORES 非字节级对齐；FILTER 非全函数面；SORTABLE 缺值「Could not find the value…」仍简化；COLLECT 为 Godis 8.8+（Redis 8.6 无）；GROUPBY 后与复杂 HAVING 交错仍简化；locale/%+ 等仍缺 |
| FT.SEARCH WITHOUTCOUNT | 领先整数=本页返回文档数（LIMIT 截断时可验） | 非完整 DIALECT 4 其它语义 |
| FT.SEARCH EXPLAINSCORE | BM25STD/TFIDF/DOCSCORE/DISMAX 嵌套 simple-string 线格式（自洽 b=0.09）；RESP3 `score` 嵌套树 | 非 RediSearch 字节级；HAMMING 仅简叶 |
...
| `MEMORY STATS` `allocator` | **`go`**；并附 `process.rss`；补齐 Redis 常用键名（`peak.percentage`、`replication.backlog`、`clients.*`、`lua.caches`/`functions.caches`/`script.VMs`、`allocator.muzzy=0`、`allocator-fragmentation.*`、`allocator-rss.*`、`rss-overhead.*` 等；数值为 Go/进程估账） | `allocator=jemalloc*`；jemalloc arena/muzzy 真账 |
...
**最后更新：** 2026-08-17（第九十一批：R4-1 扩至 **1254**（跨 1200；第八十八批跨 1000）；远期仍 **7**：jemalloc / gossip / 模块 RDB·DUMP / FUNCTION DUMP 互通 / 完整 BM25·KNN·DIALECT / R4-1 全量 / R4-2）
