...
| AGGREGATE APPLY / ADDSCORES | 算术/字符串/布尔函数；UTC `day/hour/minute/month/dayof*`、`year/monthofyear`；常用 strftime 格式 `timefmt`/`parsetime`；两坐标或四数值 `geodistance`；**`matched_terms([max])` 多值**（无 stemmer 变体）；**`split` 真多值数组**；**ADDSCORES→`@__score`**（BM25STD）；**多字段 SORTBY** + **SORTBY … MAX**（截断行；Total 预截断） | 完整 strftime 指令；stemmer 形态进 matched_terms；DIALECT 4 排序优化仍未完整；ADDSCORES 非字节级对齐 |
...
**最后更新：** 2026-08-17（第六十六批：FT.AGGREGATE 多字段 SORTBY；R4-1 扩至 **358**；对照 Redis **8.x** sidecar；远期仍 **7**：jemalloc / gossip / 模块 RDB·DUMP / FUNCTION DUMP 互通 / 完整 BM25·KNN·DIALECT / R4-1 全量 / R4-2）
