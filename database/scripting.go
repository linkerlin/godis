...
func execScriptHelp(db *DB, args [][]byte) redis.Reply {
	...
	return protocol.MakeMultiBulkReply([][]byte{
		...
		[]byte("    * ASYNC: Asynchronously flush the scripts cache."),
		[]byte("    * SYNC: Synchronously flush the scripts cache."),
		[]byte("KILL"),
		[]byte("    Kill the currently executing Lua script."),
		[]byte("LOAD <script>"),
		[]byte("    Load a script into the scripts cache without executing it."),
		[]byte("HELP"),
