...
| KNN/DIALECT 错误路径 | ✅ | 非法 DIALECT；缺 PARAMS / 非 VECTOR / dim；`HYBRID_POLICY` / **`$HYBRID_POLICY`** 枚举；**`$YIELD_DISTANCE_AS`**；**`$SHARD_K_RATIO`∈(0,1]**；**`BATCH_SIZE`/`EPSILON`/`$EF_RUNTIME` 缺值 ERR**；**VECTOR_RANGE `$radius` PARAM**（`($r)` Redis 拒→不实现）；RANGE 拒 **`$EF_RUNTIME`/hybrid 属性/未知键**；**`$EPSILON` 仅 HNSW/SVS**；空预过滤；D1 拒 tag 空格/`@f1\|f2`；比较/`ismissing`/GEOSHAPE 具体 ERR（仍非完整方言） |
...
| FT.AGGREGATE 多字段 SORTBY | ✅ 子集 | nargs 内多 property+ASC/DESC；缺字段排后；可与 MAX 组合；**未知属性→`Property … not loaded nor in schema`** |
| FT.AGGREGATE GROUPBY 属性 | ✅ 子集 | schema/LOAD/APPLY AS；**未知→`No such property`** |
| FT.SEARCH/AGGREGATE LIMIT | ✅ 子集 | 缺参→`requires two arguments`；负/非数字→`needs two numeric arguments`（防负 offset panic） |
...
| FT.SEARCH LANGUAGE | ✅ 子集 | 缺参/`SEARCH_QUERY_BAD No such language`（大小写不敏感；接受后仍走英文分词路径）；FT.CREATE 未知→`SEARCH_ADD_ARGS Invalid language` |
...
| FT.SEARCH DIALECT/SORTBY/FILTER/GEOFILTER/SLOP/PAYLOAD/INFIELDS/EXPANDER | ✅ 子集 | 缺参/范围文案对齐 Redis 8.x；SORTBY 未知→`Property … not loaded nor in schema`；GEOFILTER `<lon/lat/radius>`/`Unknown distance unit`/`Invalid GeoFilter lat/lon`；FILTER `Bad lower/upper range`；SLOP 负值接受；EXPANDER 任意名；**BM25STD_TANH_FACTOR** 缺参/`1..10000` 文案；AGGREGATE APPLY 缺参/`AS needs argument`/无 AS；GROUPBY nargs；**未知选项 `SEARCH_ARG_UNRECOGNIZED` 位次**；裸 **WITHCOUNT**；裸 **REDUCE→UNRECOGNIZED**；CREATE **STOPWORDS/TEMPORARY** PARSE_ARGS；**TEMPORARY 0** 立即无索引；CREATE **PREFIX/ON/LANGUAGE/SCORE/INDEXALL/FILTER**/无 SCHEMA；**SCORE**∈[0,1]|NaN；**LANGUAGE_FIELD/SCORE_FIELD/PAYLOAD_FIELD** 缺参；SCHEMA **Fields/type/WEIGHT/SEPARATOR/PHONETIC/VECTOR** 文案；**TIMEOUT** `SEARCH_PARSE_ARGS` 前缀 |
| FT.ALTER / DROPINDEX / SYN* | ✅ 子集 | ALTER：`ALTER must be followed by SCHEMA` / `Unknown action passed to ALTER SCHEMA` / `SEARCH_QUERY_BAD Duplicate field in schema` / 前缀 **SKIPINITIALSCAN**（不重扫既有文档）；DROPINDEX 未知参→`SEARCH_ARG_UNRECOGNIZED Unknown argument`（缺索引仍 `INDEX_NOT_FOUND`）；**FT.SYNADD** Redis 8 已移除→`No longer supported, use FT.SYNUPDATE`；SYNUPDATE/SYNDUMP 缺索引 `INDEX_NOT_FOUND`；SKIPINITIALSCAN 无 term→OK no-op |
