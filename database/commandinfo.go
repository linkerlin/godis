...
func execCommand(args [][]byte) redis.Reply {
	...
	if subCommand == "info" {
		...
		if len(args) != 1 {
			return protocol.MakeErrReply("ERR wrong number of arguments for 'command|count' command")
		}
		...
		return protocol.MakeMultiBulkReply([][]byte{
			...
			[]byte("LIST"),
			[]byte("    Return a list of all commands in this Redis server."),
			[]byte("INFO [<command-name> ...]"),
			[]byte("    Return details about multiple Redis commands."),
			[]byte("    If no command names are given, documentation details for all"),
			[]byte("    commands are returned."),
			[]byte("DOCS [<command-name> ...]"),
			[]byte("    Return documentation details about multiple Redis commands."),
			[]byte("    If no command names are given, documentation details for all"),
			[]byte("    commands are returned."),
			[]byte("GETKEYS <full-command>"),
			[]byte("    Return the keys from a full Redis command."),
			[]byte("GETKEYSANDFLAGS <full-command>"),
			[]byte("    Return the keys and the access flags from a full Redis command."),
			[]byte("HELP"),
