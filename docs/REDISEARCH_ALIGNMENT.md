...
| KNN/DIALECT 错误路径 | ✅ | 非法 DIALECT；缺 PARAMS / 非 VECTOR / dim；`HYBRID_POLICY` / **`$HYBRID_POLICY`** 枚举；**`$YIELD_DISTANCE_AS`**；**`$SHARD_K_RATIO`∈(0,1]**；**`BATCH_SIZE`/`EPSILON`/`$EF_RUNTIME` 缺值 ERR**；**VECTOR_RANGE `$radius`/`($r)`**；**`$EF_RUNTIME` 属性块**；空预过滤；D1 拒 tag 空格/`@f1\|f2`；比较/`ismissing`/GEOSHAPE 具体 ERR（仍非完整方言） |
