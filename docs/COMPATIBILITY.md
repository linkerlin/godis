...
| 完整 BM25 / 完整 KNN 方言 / 完整 DIALECT | BM25STD：WEIGHT + 文档长度 + **可测 IDF** + **多字段加权求和**；NORM min-max；**TANH + BM25STD_TANH_FACTOR**；KNN：`AS`/`$YIELD_DISTANCE_AS`、**HYBRID_POLICY**（子句与 **`$HYBRID_POLICY` 属性块**）、**BATCH_SIZE**（接受/缺值 ERR）、**EPSILON**（距离阈值）、**`$EF_RUNTIME`** 属性块、空预过滤；**VECTOR_RANGE**（DIALECT≥2；**`$radius`/`($r)` PARAM**；`$YIELD_DISTANCE_AS`/`$EPSILON` 扩半径；**`$EF_RUNTIME` 接受/校验**）；DIALECT 1/2/3 子集（GEOSHAPE≥3；**tag 空格/多字段 `@f1\|f2` 强制 ≥2**）；AGGREGATE APPLY：UTC 时间族、`parsetime`/`geodistance`、**`matched_terms([max])` 多值**、**`split` 真多值**；**FT.SEARCH WITHOUTCOUNT**；**EXPLAINSCORE**（需 WITHSCORES；BM25/TFIDF/DOCSCORE/DISMAX 子集；b 与 Redis 默认不同）+ **SORTBY WITHCOUNT** 接受 | **非**论文级完整 BM25（无 slop 罚分进打分、无全局 NORM over collection、无 stemmer 变体进 `matched_terms`）；非完整 KNN/DIALECT 4；VECTOR_RANGE 无 HNSW 近似扫描语义；`$EF_RUNTIME` 在 FLAT 上无近似副作用；EXPLAINSCORE 非字节级对齐 RediSearch；见 REDISEARCH_ALIGNMENT |
...
| VECTOR_RANGE | `@vec:[VECTOR_RANGE r|$r $q]`（DIALECT≥2；**`($r)` 括号**）；**`=>{$YIELD_DISTANCE_AS; $EPSILON}`**（ε 扩有效半径 `r*(1+ε)`）；**`$EF_RUNTIME`** 缺值/非法 ERR（FLAT 忽略）；FLAT 暴力距离过滤 | HNSW 候选扩展扫描；`$EF_RUNTIME` 近似副作用；与官方分数/半径边界字节级对齐 |
...
**最后更新：** 2026-08-17（第六十二批：VECTOR_RANGE `$radius`/`($r)` + `$EF_RUNTIME`；KNN `$HYBRID_POLICY`；R4-1 扩至 **296**；对照 Redis **8.x** sidecar；远期仍 **7**：jemalloc / gossip / 模块 RDB·DUMP / FUNCTION DUMP 互通 / 完整 BM25·KNN·DIALECT / R4-1 全量 / R4-2）
