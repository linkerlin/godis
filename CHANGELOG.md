...
- 第六十二批（远期子集续研）：FT.SEARCH **VECTOR_RANGE `$radius` PARAM**（含 `($r)` 括号形）+ 属性块 **`$EF_RUNTIME`**（缺值/非法 ERR；FLAT 接受后忽略）；KNN 属性块 **`$HYBRID_POLICY`**（缺值/非法 ERR）；R4-1 扩至 **296** 条实跑断言（ZMSCORE/ZREVRANGE/SMISMEMBER/HINCRBYFLOAT/PEXPIRETIME/ZDIFFSTORE）；远期仍 7 项，不宣称完整 KNN/DIALECT 或全量 sidecar
...
- KNN `BATCH_SIZE`/`EPSILON`（缺值 ERR；`EPSILON` 距离过滤；`$BATCH_SIZE`/`$EPSILON`/`$EF_RUNTIME`/`$HYBRID_POLICY` 属性块）；**VECTOR_RANGE**（含 **`$radius` PARAM** / `($r)`；`$EF_RUNTIME` 接受）；R4-1 **296** 条（ZMSCORE/ZREVRANGE/SMISMEMBER/HINCRBYFLOAT/PEXPIRETIME/ZDIFFSTORE）
