...
**M2 里程碑：** 至 **M2bj** 已含 FT.SEARCH 默认 LIMIT、CLIENT flags `T`/`O`+`redir=`、INFO 从节点字段、CONFIG aof-use-rdb-preamble/masterauth 等。仍延期：集群 CRC16/MOVED、HLL 互通、真 HNSW、完整 FAILOVER、精确 `used_memory`、FUNCTION DUMP 官方互通、MIGRATE 等（见计划文档）。
