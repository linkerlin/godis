...
| AGGREGATE APPLY / ADDSCORES | 算术/字符串/布尔函数；UTC `day/hour/minute/month/dayof*`、`year/monthofyear`；常用 strftime 格式 `timefmt`/`parsetime`；两坐标或四数值 `geodistance`；**`matched_terms([max])` 多值**（无 stemmer 变体）；**`split` 真多值**（**charset sep/strip**、空段丢弃）；**`format` PARSE_ARGS**；**`to_str`/`to_number`**；**ADDSCORES→`@__score`**（BM25STD）；**多字段 SORTBY** + **SORTBY … MAX**；**LOAD … AS** / **`@__key`**（有 LOAD 时投影；无 LOAD 仅 SORTABLE）；**APPLY/FILTER 未入管道→`Property not loaded nor in pipeline`**（GROUPBY/REDUCE 仍可读全文档）；**REDUCE 名称/参数校验**（`No such reducer`/`Count accepts 0…`/`Missing arguments`/`QUANTILE`/`RANDOM_SAMPLE`/`FIRST_VALUE BY`）；**FT.CREATE FILTER**；**WITHCURSOR MAXIDLE**；**TIMEOUT soft 部分结果** + **ON_TIMEOUT RETURN\|FAIL** | 完整 strftime 指令；stemmer 形态进 matched_terms；DIALECT 4 排序优化仍未完整；ADDSCORES 非字节级对齐；FILTER 非全函数面；SORTABLE 缺值「Could not find the value…」仍简化；COLLECT 为 Godis 8.8+（Redis 8.6 无） |
...
**最后更新：** 2026-08-17（第七十二批：APPLY **`split` charset sep/strip** + **`format` PARSE_ARGS**；R4-1 扩至 **443**；对照 Redis **8.x** sidecar；远期仍 **7**：jemalloc / gossip / 模块 RDB·DUMP / FUNCTION DUMP 互通 / 完整 BM25·KNN·DIALECT / R4-1 全量 / R4-2）
