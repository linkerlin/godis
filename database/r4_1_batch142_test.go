...
func TestR41Batch142Extras(t *testing.T) {
	...
	asserts.AssertIntReply(t, db.Exec(nil, utils.ToCmdLine("RPUSH", "b142l", "10", "20", "30", "40", "50", "60", "70")), 7)
	asserts.AssertMultiBulkReply(t, db.Exec(nil, utils.ToCmdLine("LPOP", "b142l", "1")), []string{"10"})
	asserts.AssertMultiBulkReply(t, db.Exec(nil, utils.ToCmdLine("LRANGE", "b142l", "0", "-1")), []string{"20", "30", "40", "50", "60", "70"})
	asserts.AssertBulkReply(t, db.Exec(nil, utils.ToCmdLine("LMOVE", "b142l", "b142l2", "RIGHT", "RIGHT")), "70")
	asserts.AssertMultiBulkReply(t, db.Exec(nil, utils.ToCmdLine("LRANGE", "b142l", "0", "-1")), []string{"20", "30", "40", "50", "60"})
	asserts.AssertMultiBulkReply(t, db.Exec(nil, utils.ToCmdLine("LRANGE", "b142l2", "0", "-1")), []string{"70"})

	asserts.AssertIntReply(t, db.Exec(nil, utils.ToCmdLine("RPUSH", "b142ins", "top", "bot")), 2)
	asserts.AssertIntReply(t, db.Exec(nil, utils.ToCmdLine("LINSERT", "b142ins", "BEFORE", "bot", "mid")), 3)
	asserts.AssertMultiBulkReply(t, db.Exec(nil, utils.ToCmdLine("LRANGE", "b142ins", "0", "-1")), []string{"top", "mid", "bot"})

	asserts.AssertIntReply(t, db.Exec(nil, utils.ToCmdLine("RPUSH", "b142lp", "r", "s", "r", "s", "r")), 5)
	asserts.AssertMultiBulkReply(t, db.Exec(nil, utils.ToCmdLine("LPOS", "b142lp", "r", "COUNT", "0")), []string{"0", "2", "4"})
	asserts.AssertIntReply(t, db.Exec(nil, utils.ToCmdLine("LPOS", "b142lp", "r", "RANK", "-1")), 4)
	asserts.AssertMultiBulkReply(t, db.Exec(nil, utils.ToCmdLine("LPOS", "b142lp", "r", "COUNT", "2", "RANK", "2")), []string{"2", "4"})

	asserts.AssertIntReply(t, db.Exec(nil, utils.ToCmdLine("RPUSH", "b142lmp", "v1", "v2", "v3", "v4")), 4)
	lmp := db.Exec(nil, utils.ToCmdLine("LMPOP", "1", "b142lmp", "LEFT", "COUNT", "2"))
	if protocol.IsErrorReply(lmp) || !strings.Contains(string(lmp.ToBytes()), "v1") {
		t.Fatalf("LMPOP LEFT COUNT 2: %s", lmp.ToBytes())
	}
	asserts.AssertIntReply(t, db.Exec(nil, utils.ToCmdLine("LLEN", "b142lmp")), 2)

	asserts.AssertIntReply(t, db.Exec(nil, utils.ToCmdLine("RPUSH", "b142bl", "up", "down", "left")), 3)
	...
	asserts.AssertMultiBulkReply(t, db.Exec(nil, utils.ToCmdLine("LRANGE", "b142bl", "0", "-1")), []string{"down", "left"})
	asserts.AssertMultiBulkReply(t, db.Exec(nil, utils.ToCmdLine("LRANGE", "b142bld", "0", "-1")), []string{"up"})

	asserts.AssertIntReply(t, db.Exec(nil, utils.ToCmdLine("RPUSH", "b142lt", "a", "b", "c", "d", "e")), 5)
	asserts.AssertStatusReply(t, db.Exec(nil, utils.ToCmdLine("LSET", "b142lt", "4", "X")), "OK")
	asserts.AssertIntReply(t, db.Exec(nil, utils.ToCmdLine("LREM", "b142lt", "1", "c")), 1)
	asserts.AssertStatusReply(t, db.Exec(nil, utils.ToCmdLine("LTRIM", "b142lt", "1", "3")), "OK")
	asserts.AssertMultiBulkReply(t, db.Exec(nil, utils.ToCmdLine("LRANGE", "b142lt", "0", "-1")), []string{"b", "d", "X"})
	asserts.AssertIntReply(t, db.Exec(nil, utils.ToCmdLine("LPUSHX", "b142lx", "v")), 0)
	asserts.AssertIntReply(t, db.Exec(nil, utils.ToCmdLine("RPUSH", "b142lx", "n")), 1)
	asserts.AssertIntReply(t, db.Exec(nil, utils.ToCmdLine("LPUSHX", "b142lx", "m")), 2)
	asserts.AssertIntReply(t, db.Exec(nil, utils.ToCmdLine("RPUSHX", "b142lx", "o")), 3)
	asserts.AssertMultiBulkReply(t, db.Exec(nil, utils.ToCmdLine("LRANGE", "b142lx", "0", "-1")), []string{"m", "n", "o"})

	asserts.AssertIntReply(t, db.Exec(nil, utils.ToCmdLine("ZADD", "b142zs", "6", "a", "11", "b", "16", "c", "21", "d", "26", "e")), 5)
	asserts.AssertMultiBulkReply(t, db.Exec(nil, utils.ToCmdLine("ZRANGE", "b142zs", "11", "21", "BYSCORE", "WITHSCORES", "LIMIT", "1", "2")),
		[]string{"c", "16", "d", "21"})
	asserts.AssertIntReply(t, db.Exec(nil, utils.ToCmdLine("ZRANGESTORE", "b142zrs", "b142zs", "11", "26", "BYSCORE")), 4)
	asserts.AssertMultiBulkReply(t, db.Exec(nil, utils.ToCmdLine("ZRANGE", "b142zrs", "0", "-1")), []string{"b", "c", "d", "e"})
	asserts.AssertMultiBulkReply(t, db.Exec(nil, utils.ToCmdLine("ZREVRANGEBYSCORE", "b142zs", "21", "11", "WITHSCORES", "LIMIT", "0", "2")),
		[]string{"d", "21", "c", "16"})
	asserts.AssertMultiBulkReply(t, db.Exec(nil, utils.ToCmdLine("ZPOPMAX", "b142zs")), []string{"e", "26"})
	asserts.AssertIntReply(t, db.Exec(nil, utils.ToCmdLine("ZUNIONSTORE", "b142zu", "1", "b142zs", "WEIGHTS", "2", "AGGREGATE", "MIN")), 4)
	asserts.AssertMultiBulkReply(t, db.Exec(nil, utils.ToCmdLine("ZRANGE", "b142zu", "0", "-1", "WITHSCORES")),
		[]string{"a", "12", "b", "22", "c", "32", "d", "42"})

	asserts.AssertIntReply(t, db.Exec(nil, utils.ToCmdLine("ZADD", "b142zlex", "0", "e", "0", "f", "0", "g", "0", "h")), 4)
	asserts.AssertMultiBulkReply(t, db.Exec(nil, utils.ToCmdLine("ZRANGE", "b142zlex", "[e", "(h", "BYLEX")), []string{"e", "f", "g"})
	asserts.AssertIntReply(t, db.Exec(nil, utils.ToCmdLine("ZRANGESTORE", "b142zrs2", "b142zlex", "[f", "(h", "BYLEX")), 2)
	asserts.AssertMultiBulkReply(t, db.Exec(nil, utils.ToCmdLine("ZRANGE", "b142zrs2", "0", "-1")), []string{"f", "g"})
	asserts.AssertMultiBulkReply(t, db.Exec(nil, utils.ToCmdLine("ZREVRANGEBYLEX", "b142zlex", "(h", "[f")), []string{"g", "f"})
	asserts.AssertIntReply(t, db.Exec(nil, utils.ToCmdLine("ZREMRANGEBYLEX", "b142zlex", "[e", "(g")), 2)
	asserts.AssertMultiBulkReply(t, db.Exec(nil, utils.ToCmdLine("ZRANGE", "b142zlex", "0", "-1")), []string{"g", "h"})

	asserts.AssertIntReply(t, db.Exec(nil, utils.ToCmdLine("ZADD", "b142z1", "11", "a", "18", "b", "9", "c")), 3)
	asserts.AssertIntReply(t, db.Exec(nil, utils.ToCmdLine("ZADD", "b142z2", "8", "b", "5", "c", "14", "d")), 3)
	...
	asserts.AssertIntReply(t, db.Exec(nil, utils.ToCmdLine("ZINTERSTORE", "b142zi", "2", "b142z1", "b142z2", "WEIGHTS", "2", "1", "AGGREGATE", "MAX")), 2)
	...
	asserts.AssertIntReply(t, db.Exec(nil, utils.ToCmdLine("ZADD", "b142zm", "8", "a", "17", "k", "12", "b")), 3)
	...
	asserts.AssertIntReply(t, db.Exec(nil, utils.ToCmdLine("ZADD", "b142zm", "GT", "60", "k")), 0)
	asserts.AssertIntReply(t, db.Exec(nil, utils.ToCmdLine("ZADD", "b142zm", "LT", "3", "b")), 0)
	asserts.AssertBulkReply(t, db.Exec(nil, utils.ToCmdLine("ZADD", "b142zm", "INCR", "0.5", "b")), "3.5")
	asserts.AssertBulkReply(t, db.Exec(nil, utils.ToCmdLine("ZSCORE", "b142zm", "k")), "60")
	...
	bf := db.Exec(nil, utils.ToCmdLine("BITFIELD", "b142bf", "SET", "u8", "0", "55"))
	...
	asserts.AssertIntReply(t, db.Exec(nil, utils.ToCmdLine("HSET", "b142h", "f1", "hh", "f2", "14", "f3", "gg")), 3)
	...
	hpgt := db.Exec(nil, utils.ToCmdLine("HPEXPIRE", "b142h", "3000", "GT", "FIELDS", "1", "f1"))
	...
	he := db.Exec(nil, utils.ToCmdLine("HGETEX", "b142h", "EX", "40", "FIELDS", "1", "f2"))
	if protocol.IsErrorReply(he) || !strings.Contains(string(he.ToBytes()), "14") {
		t.Fatalf("HGETEX EX: %s", he.ToBytes())
	}
	asserts.AssertIntReply(t, db.Exec(nil, utils.ToCmdLine("HINCRBY", "b142h", "f2", "8")), 22)
	asserts.AssertIntReply(t, db.Exec(nil, utils.ToCmdLine("HLEN", "b142h")), 3)

	asserts.AssertIntReply(t, db.Exec(nil, utils.ToCmdLine("SADD", "b142sa", "p", "q", "r")), 3)
	asserts.AssertIntReply(t, db.Exec(nil, utils.ToCmdLine("SADD", "b142sb", "r", "s")), 2)
	...
	asserts.AssertMultiBulkReply(t, db.Exec(nil, utils.ToCmdLine("SUNION", "b142ss")), []string{"r"})
	...
	gs := db.Exec(nil, utils.ToCmdLine("GEOSEARCH", "b142g", "FROMLONLAT", "15", "37.5", "BYBOX", "70", "70", "km", "ASC", "COUNT", "1"))
	if protocol.IsErrorReply(gs) || !strings.Contains(string(gs.ToBytes()), "Catania") {
		t.Fatalf("GEOSEARCH BYBOX: %s", gs.ToBytes())
	}

	asserts.AssertStatusReply(t, db.Exec(nil, utils.ToCmdLine("SET", "b142s1", "database")), "OK")
	asserts.AssertStatusReply(t, db.Exec(nil, utils.ToCmdLine("SET", "b142s2", "base")), "OK")
	asserts.AssertBulkReply(t, db.Exec(nil, utils.ToCmdLine("LCS", "b142s1", "b142s2")), "base")
	asserts.AssertIntReply(t, db.Exec(nil, utils.ToCmdLine("LCS", "b142s1", "b142s2", "LEN")), 4)
	asserts.AssertIntReply(t, db.Exec(nil, utils.ToCmdLine("RPUSH", "b142sort", "11", "4", "20", "7", "15")), 5)
	asserts.AssertMultiBulkReply(t, db.Exec(nil, utils.ToCmdLine("SORT", "b142sort", "DESC", "LIMIT", "1", "2")), []string{"15", "11"})
	asserts.AssertIntReply(t, db.Exec(nil, utils.ToCmdLine("PFADD", "b142p1", "a")), 1)
	asserts.AssertIntReply(t, db.Exec(nil, utils.ToCmdLine("PFADD", "b142p2", "b")), 1)
