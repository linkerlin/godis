...
**M2 里程碑：** 至 **M2ci** 已含 **M2cg**（CONFIG 持久化/AOF 改写桩、CLUSTER GETKEYSINSLOT/FORGET/RESET/SAVECONFIG、INFO unblocked_clients/total_watched_keys/cpu main_thread、ACL CAT @json/@search/@vector、Lua ZINCRBY/HINCRBYFLOAT/INCRBYFLOAT number）、**M2ch**（CONFIG io-threads/repl-diskless/maxmemory-samples/tracking-table-max-keys、INFO cluster 段、CLIENT LIST peerid、CLUSTER FLUSHSLOTS/MYSHARDID、Lua ZRANK/ZREVRANK WITHSCORE number）与 **M2ci**（CONFIG repl-backlog-ttl 等、DEBUG OBJECT、INFO total_blocking_keys、ACL CAT @bloom/@cuckoo/@timeseries、CLUSTER ADDSLOTSRANGE/DELSLOTSRANGE）等。仍延期：集群 CRC16/MOVED、HLL 互通、真 HNSW、完整 FAILOVER、精确 `used_memory`、FUNCTION DUMP 官方互通、MIGRATE 等（见计划文档）。
