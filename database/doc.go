...
/*
[Server](https://github.com/linkerlin/godis/blob/master/database/database.go) is an implemention of interface [DB](https://github.com/linkerlin/godis/blob/master/interface/database/db.go).

[server.Handler](https://github.com/linkerlin/godis/blob/master/redis/server/server.go) holds an instance of Server as storage engine, and pass command line to Server through db.Exec method.
...
[godis.DB.Exec](https://github.com/linkerlin/godis/blob/master/database/single_db.go) handles transaction control command (such as watch, multi, exec) itself, and invokes DB.execNormalCommand to handle normal commands. The word, normal command, is commands which read or write limited keys, can execute within transaction, and supports rollback. For example, get, set, lpush are normal commands, while flushdb, keys are not.
