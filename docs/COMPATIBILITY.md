...
**M2 里程碑：** 至 **M2cd** 已含 CONFIG client-buffer/min-replicas、INFO forks/cow、Lua HSCAN/ZSCAN→map、CLUSTER COUNT-FAILURE-REPORTS/BUMPEPOCH、Z*STORE GETKEYS 等。仍延期：集群 CRC16/MOVED、HLL 互通、真 HNSW、完整 FAILOVER、精确 `used_memory`、FUNCTION DUMP 官方互通、MIGRATE 等（见计划文档）。
